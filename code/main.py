...
class Produto:
    ...
    def verificar_passivel_de_troca(self):
        if self._passivel_de_troca:
            ...
            decisao = input(
                "Digite '1' se o produto for não passível de troca, devolução ou cancelamento\nDigite '2' o produto não for passível de troca por defeito: ")
            if decisao == '1':
                ...
                if decisao == '1':
                    print(
                        "Esse produto não é passível de devolução, sendo sua troca efetuada apenas por defeito e pelo mesmo produto.")
                if decisao == '2':
            ...
            if decisao == '2':
                ...
                if decisao == '2':
                    print(
                        "Será necessário acionar o fabricante, após isso as peças faltantes serão repostas")
                if decisao == '3':
...
class Troca_Loja_Fisica(Fluxo_de_Troca):
    def realizar_troca(self):
        cprint(
            "================= Começo Troca Loja Física =================", 'red', 'on_cyan')
        ...
        if decisao == '1':
            print("Realizando troca em loja física para o produto '{}'" .format(
                self._produto._nome))
        if decisao == '2':
            novoProduto = input("Digite o nome do produto desejado: ")
            print("Realizando troca em loja física do produto '{}' para o produto '{}'".format(
                self._produto._nome, novoProduto))
        if decisao == '3':
            print("Infelizmente a troca em loja física para o produto '{}' não é possível de ser realizada" .format(
                self._produto._nome))
        cprint(
            "================= Fim Troca Loja Física =================", 'red', 'on_cyan')
        pass
    ...
    def printaCondicoes(self):
        ...
        condicoes.append(
            "A embalagem é a original, sem uso e com a etiqueta de identificação fixada?")
        condicoes.append("Possui a nota fiscal do produto?")
        condicoes.append(
            "Seu produto é de valor igual ou maior do que o desejado?")
        condicoes.append(
            "Se o produto desejado for de valor maior, você dejeja completar o valor faltante?")
...
class Troca_Marketplace(Fluxo_de_Troca):
    def realizar_troca(self):
        cprint(
            "================= Começo Troca Marketplace =================", 'red', 'on_cyan')
        print("Produto acordado com o parceiro, realizando troca em marketplace para o produto:",
              self._produto._nome)
        cprint(
            "================= Fim Troca Marketplace =================", 'red', 'on_cyan')
        pass
...
class Troca_Site_Whatsapp(Fluxo_de_Troca):
    def realizar_troca(self):
        cprint(
            "================= Começo Troca Site/Whatsapp =================", 'red', 'on_cyan')
        decisao = input(
            "Digite '1' para Devolução/Arrependimento ou Insatisfação com o Produto\nDigite '2' para Troca por defeito: ")
        if decisao == '1':
            self.printaCondicoes("Troca_Site_Whatsapp_1")
            decisao = input(
                "Digite '1' se seu produto atende a todas as condições e você deseja o reembolso e '2' se você não deseja: ")
            if decisao == '2':
                print("Infelizmente o reembolso para o produto '{}' não é possível de ser realizado" .format(
                    self._produto._nome))
            if decisao == '1':
                print(
                    "Realizando reembolso para o produto {}, confira seu e-mail" .format(self._produto._nome))
            decisao = 13
        if decisao == '2':
            decisao = input(
                "Para realizar a Troca/Reembolso pela loja física, digite '1'\nPelos Correios, digite '2': ")
            if decisao == '1':
            ...
            if decisao == '2':
                ...
                if decisao == '1':
                    print(
                        "Realizando reembolso para o produto {}, confira seu e-mail" .format(self._produto._nome))
        cprint(
            "================= Fim Troca Site/Whatsapp =================", 'red', 'on_cyan')
        pass
    ...
    def printaCondicoes(self, qualTroca):
        if qualTroca == "Troca_Loja_Fisica":
            condicoes = []
            condicoes.append(
                "90 dias corridos já se passaram a partir da compra?")
            condicoes.append(
                "A embalagem é a original, sem uso e com a etiqueta de identificação fixada?")
            condicoes.append("Possui a nota fiscal do produto?")
            condicoes.append(
        ...
        if qualTroca == "Troca_Site_Whatsapp_1":
            condicoes = []
            condicoes.append(
                "07 dias corridos a partir da entrega já passaram?")
            condicoes.append(
                "O cancelamento e reembolso da compra será efetuado somente após o recebimento e conferência do produto em nosso centro de distribuição")
            condicoes.append(
                "A postagem deverá ser feita em uma agência, com código fornecido após preenchimento de um formulário e, após isso, poderá ser escolhida a forma de reembolso")
            condicoes.append(
                "Devolução pode ser parcial (sem frete) ou total (com frete incluso)")
        ...
        if qualTroca == "Troca_Site_Whatsapp_2":
            ...
            condicoes.append("O produto desejado faz parte dessa loja?")
            condicoes.append(
                "Em casa de defeito no produto, será necessário o envio de imagens que comprovem tal situação")
        for i in condicoes:
            cprint(i, 'green', 'on_red')
...
adquirido_por_numero = input(
    "Como o produto foi adquirido? Digite 1 para Site/Whatsapp, 2 para marketplace e 3 para loja física\n> ")
adquirido_por = ""
...
adquirido_em = input(
    "Qual a data que o produto foi adquirido? (dd/mm/yyyy)\n> ")
...
print('Lego, playmobil, quebra-cabeças e jogos, produtos surpresa, bicicletas, puericultura pesada (carrinhos, banheiras, cadeirinhas, etc), games, miniveículos e miniveículos elétricos, Puericultura leve (bico de chupeta ou mamadeira, mamadeira, chupeta, mordedor, etc), papelaria, e vestuário, Brinquedos eletrônicos, Fantasias.')
passivel_de_troca_str = input(
    "\nSeu produto está incluido nessa lista? Digite 's' para sim e 'n' para não.\n> ")
